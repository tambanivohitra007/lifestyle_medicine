    ...
    <div className="card border-l-4 border-l-primary-500">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className={`p-2 rounded-lg ${getCompletionColor(completionScore)}`}>
            <Lightbulb className="w-5 h-5" />
          </div>
          <div>
            <h3 className="font-semibold text-gray-900">Treatment Guide Workflow</h3>
            <p className="text-sm text-gray-500">Follow this guide to create a complete treatment protocol</p>
          </div>
        </div>
        <div className="flex items-center gap-3">
        ...
        <div className="space-y-6">
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <h4 className="font-medium text-gray-900">Document the Condition</h4>
              <span className="text-xs text-gray-500">({requiredSectionsComplete}/{requiredSectionsTotal} complete)</span>
            </div>
            <div className="ml-8 space-y-2">
              {REQUIRED_SECTIONS.map((section) => {
                const isComplete = existingSectionTypes.has(section.type);
                return (
                  <div key={section.type} className="flex items-center justify-between p-2 rounded-lg hover:bg-gray-50">
                    <div className="flex items-center gap-3">
                      {isComplete ? (
                      ...
                      <div>
                        <span className={`font-medium ${isComplete ? 'text-gray-700' : 'text-gray-900'}`}>
                          {section.label}
                        </span>
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              <span className="flex items-center justify-center w-6 h-6 rounded-full bg-primary-100 text-primary-700 text-sm font-bold">2</span>
              <h4 className="font-medium text-gray-900">Add Lifestyle Interventions</h4>
              <span className="text-xs text-gray-500">({coreDomainsWithInterventions}/{coreDomainsTotal} core domains)</span>
            </div>
            <div className="ml-8">
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        {hasIntervention && (
                          <span className="ml-auto text-xs bg-green-200 text-green-700 px-1.5 rounded">
                            {interventionCount}
                          </span>
                        )}
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              <span className="flex items-center justify-center w-6 h-6 rounded-full bg-primary-100 text-primary-700 text-sm font-bold">3</span>
              <h4 className="font-medium text-gray-900">Add Evidence & References</h4>
            </div>
            <div className="ml-8 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <p className="text-sm text-blue-700">
                Evidence entries are added to each intervention individually. Edit an intervention to add supporting research and citations.
              </p>
              <Link
                to="/evidence"
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              <span className="flex items-center justify-center w-6 h-6 rounded-full bg-primary-100 text-primary-700 text-sm font-bold">4</span>
              <h4 className="font-medium text-gray-900">Add Spiritual Guidance</h4>
              <span className="text-xs text-gray-500">
                {hasScriptures ? (
                  <span className="text-green-600">({scriptures.length} scriptures)</span>
                ) : (
                  <span className="text-yellow-600">(recommended)</span>
                )}
              </span>
            </div>
            <div className="ml-8 flex items-center justify-between p-2 rounded-lg hover:bg-gray-50">
              <div className="flex items-center gap-3">
                {hasScriptures ? (
                  <CheckCircle2 className="w-5 h-5 text-green-500" />
                ) : (
                ...
                <div>
                  <span className="font-medium text-gray-700">Bible & Spirit of Prophecy</span>
                  <p className="text-xs text-gray-500">Scripture passages and Ellen White quotes</p>
                </div>
              </div>
              <Link
                ...
                className="text-primary-600 hover:text-primary-700 text-sm font-medium flex items-center gap-1"
              >
                {hasScriptures ? 'Add More' : <><Plus className="w-4 h-4" />Add</>}
              </Link>
            </div>
          </div>
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              <span className="flex items-center justify-center w-6 h-6 rounded-full bg-gray-100 text-gray-600 text-sm font-bold">5</span>
              <h4 className="font-medium text-gray-700">Add Recipes (Optional)</h4>
              {hasRecipes && (
                <span className="text-xs text-green-600">({recipes.length} recipes)</span>
              )}
            </div>
            <div className="ml-8 flex items-center justify-between p-2 rounded-lg hover:bg-gray-50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <span className="font-medium text-gray-700">Therapeutic Recipes</span>
                  <p className="text-xs text-gray-500">Culinary medicine recipes for this condition</p>
                </div>
              </div>
              <Link
                ...
                className="text-primary-600 hover:text-primary-700 text-sm font-medium flex items-center gap-1"
              >
                {hasRecipes ? 'Add More' : <><Plus className="w-4 h-4" />Add</>}
              </Link>
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-gray-200">
            <h4 className="text-sm font-medium text-gray-500 mb-3">Optional Sections</h4>
