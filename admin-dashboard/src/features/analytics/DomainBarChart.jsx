...
const DomainBarChart = ({ data, loading }) => {
  ...
  if (!data || data.length === 0) {
    return (
      <div className="bg-white rounded-lg shadow p-6">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('analytics:charts.interventionsByDomain')}</h3>
        <div className="h-[300px] flex items-center justify-center text-gray-500">
          {t('analytics:empty.noDomainData')}
        </div>
      </div>
    );
  }
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white px-3 py-2 border border-gray-200 rounded-lg shadow-lg">
          ...
          <p className="text-sm text-gray-600">{payload[0].value} {t('analytics:labels.interventions')}</p>
        </div>
      );
    }
    return null;
